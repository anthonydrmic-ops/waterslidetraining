      ...
      <nav className="fixed top-0 left-0 right-0 z-40 px-4 pt-5">
        <div className="max-w-[1200px] mx-auto">
          <div className="nav-glass rounded-full px-4 py-3 flex items-center justify-between">
            ...
            <Link
              href="/sign-in"
              className="px-4 py-2 rounded-full bg-stone-100 text-stone-600 text-xs font-medium hover:bg-stone-200 transition-all duration-300"
            >
              Sign In
            </Link>
          </div>
      ...
      <div className="max-w-[1200px] mx-auto px-6 pt-28 pb-16">
        <motion.div variants={stagger} initial="hidden" animate="show">
          ...
          <motion.div variants={fadeUp} className="text-center">
            ...
            >
              {loading ? "Redirecting to checkout..." : `Purchase ${tiers.find(t => t.id === selectedTier)?.name} Plan`}
              <ArrowRight
                size={16}
                weight="bold"
            ...
            <Link
              href="/train"
              className="inline-flex items-center gap-2 mt-4 px-5 py-2.5 rounded-full bg-stone-800 text-white text-xs font-medium hover:bg-stone-700 transition-all duration-300"
            >
              Dev: Go to Course
